...
<template>
  <div :class="`z-10 fixed h-full  w-full bg-black bg-opacity-30 ${isSideBarVisible === true ? 'visible' : 'hidden'}`" @click="hideSideBar()" />
  <div class="fixed z-20 h-full bg-red-500">
    <ul :class="`wrapper ${isSideBarVisible === true ? 'open' : 'close'}`">
      <li>
        <NuxtLink class="w-full flex flex-row place-content-between nav-btn" to="/boulders" @click="hideSideBar()">
